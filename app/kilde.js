/*
 * Genesys Demo Site henter sin egen kode.
 *
 * Runen BAERER ikke appen - den henter den fra GitHub-taggen `vN`. Moensteret
 * er taget uaendret fra Sagu, hvor det har koert siden v47, og grunden er den
 * samme: stod taggen i install-scriptet, ville en ny app-udgave kraeve en ny
 * rune, og Andreas skulle gennem panelets to trin ved hver eneste udgivelse
 * for at flytte ét tal i en YAML.
 *
 ...
 *    et demo-site ti minutter foer en demo.
 ...
 *    ligger app/ under `.gds-gammel` - og startup-kommandoen saetter den
 ...
 *    naar en udgivelse er daarlig: saet tallet i panelet, genstart. Det er
 *    ogsaa vejen til at fryse sitet paa en kendt udgave dagen foer en stor demo.
 *
 ...
 * tilfaeldigvis naevner foerst (GitHub sorterer tags ALFABETISK, saa `v9`
 * staar efter `v80`).
 *
 ...
 * skal man vide, at Genesys Demo Site ikke gjorde, hvad der stod.
 */
 ...
 * install-script - og saa staar tallet i
...
function hentSvar(url, forsoeg, ved) {
  return new Promise((ok, nej) => {
    const req = https.get(url, {
      headers: { 'user-agent': 'genesys-demo-site-opdatering', accept: '*/*' },
      timeout: TIMEOUT_MS,
    }, (res) => {
 ...
 * den foerste. Fejlen bider foerst ved v10, og det er praecis derfor, den skal
 * skrives rigtigt nu - den dukker op om et halvt aar, hvor ingen leder her.
 */
...
/**
 * Henter og pakker `vN` ud i .gds-ny/ og returnerer stien til app-mappen.
 *
...
async function hentUdgave(version) {
   ...
   * find den app-mappe, der FINDES (RUNE-ERFARINGER). Arkivet begynder
...
/**
 * Er det her en hel app - og er det den, vi bad om?
 *
 * Sidernes skabeloner og css staar med i listen med vilje: de er ikke
 * server-filer, og en pakning, der glemte dem, ville give en server, der
 * starter fint og foerst faelder, naar nogen aabner sitet.
 *
 ...
 *
 * Doer processen mellem dem, ligger den gamle app under `.gds-gammel`, og
...
async function opdater(env = process.env) {
  ...
  if (maal < FOERSTE_SELVHENTENDE) {
    advar(`v${maal} er fra foer appen kunne hente sin egen kode.`);
    advar('En genstart opdaterer ikke derfra - brug »Opdater demo-sitet« i panelet for at komme videre.');
  }
